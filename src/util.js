...
class Util {
  ...
  isValid(name, value, callback, reject) {
    ...
    if (!value || value.toString().length === 0) {
      const error = new Error(
        `spotinst-sdk-nodejs: invalid or missing field value (${name}=${value})`
      );
  ...
  resolveOnSuccess(resp, callback, resolve) {
    typeof callback === "function" ? callback(null, resp) : resolve(resp);
  }

  rejectOnFailure(err, callback, reject) {
    typeof callback === "function" ? callback(err) : reject(err);
  }
}
